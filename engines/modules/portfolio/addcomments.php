...
if( $CN_HALT ) {
	...
	$row = $db->super_query( "SELECT
			id, portfolio_id, user_id, DATE_FORMAT(date,'%Y-%m-%d') as date, text, ip, is_register
		FROM " . PREFIX . "_portfolio_comments
		WHERE portfolio_id = '$post_id'
	...
	if( ! $CN_HALT ) {
		...
		if( $update_comments AND $parent_id == 0 ) {

			$comments = $db->safesql( $row['text'] ) . "<br /><br />" . $comments;
			$db->query( "UPDATE " . PREFIX . "_portfolio_comments set text='{$comments}'  WHERE id='{$row['id']}'" );

		} else {

			if( $is_logged ) $db->query( "INSERT INTO " . PREFIX . "_portfolio_comments ( portfolio_id, user_id, date, autor, email, text, ip, is_register ) values ( '$post_id', '$member_id[user_id]', '$time', '$name', '$mail', '$comments', '$_IP', '1' )" );
			else $db->query( "INSERT INTO " . PREFIX . "_portfolio_comments ( portfolio_id, date, autor, email, text, ip, is_register ) values ( '$post_id', '$time', '$name', '$mail', '$comments', '$_IP', '0' )" );

			// обновление количества комментариев в новостях
			if( $where_approve ) $db->query( "UPDATE " . PREFIX . "_portfolio set comm_num=comm_num+1 where id='$post_id'" );
