...
class DLE_Comments {
	...
	function build_comments( $template, $area )
	{
		...
		while ( $row = $this->db->get_row( $sql_result ) ) {
			...
			{
				$tpl->set( '[com-edit]', "<a onclick=\"return dropdownmenu(this, event, PortfolioMenuCommBuild('" . $row['id'] . "', '" . $area . "'), '170px')\" onmouseout=\"delayhidemenu()\" href=\"" . $config['http_home_url'] . "index.php?do=portfolio&amp;act=comments&amp;act=edit&amp;id=" . $row['id'] . "&amp;area=" . $area ."\">" );
				$tpl->set( '[/com-edit]', "</a>" );
			...
			{
				$tpl->set( '[com-del]', "<a href=\"javascript:confirmDelete('" . $config['http_home_url'] . "index.php?do=portfolio&amp;act=comments&amp;sub_act=del&amp;id=" . $row['id'] . "&amp;dle_allow_hash=" . $dle_login_hash . "&amp;area=" . $area ."')\">" );
				$tpl->set( '[/com-del]', "</a>" );
			} else
		...
		if( $this->comments_per_pages ) {
			...
			if( $enpages_count <= 10 ) {
				...
				if( $start >= 2 ) {
					...
					} else $pages .= "<a href=\"$PHP_SELF?cstart=1&amp;{$link}#comment\">1</a> ... ";
