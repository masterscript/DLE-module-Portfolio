...
define ( 'ENGINE_DIR',	dirname ( dirname ( dirname ( dirname ( __FILE__ )))));
...
$allowed_areas = array(
					...
					'news' => array (
									'comments_table' => 'portfolio_comments',
									),
...
<div align="right" style="width:99%;padding-top:5px;"><input class=bbcodes title="$lang[bb_t_apply]" type=button onclick="portfolio_ajax_save_comm_edit('{$id}', '{$area}'); return false;" value="$lang[bb_b_apply]">
<input class=bbcodes title="$lang[bb_t_cancel]" type=button onclick="portfolio_ajax_cancel_comm_edit('{$id}'); return false;" value="$lang[bb_b_cancel]">
</div>
</form>
HTML;
}
...
{
	$row = $db->super_query( "SELECT id, portfolio_id, text, is_register, autor FROM " . PREFIX . "_{$allowed_areas[$area]['comments_table']} where id = '$id'" );
	$row[ 'post_id' ] = $row[ 'portfolio_id' ];
	...
	if( ! $row['approve'] ) $db->query( "UPDATE " . PREFIX . "_portfolio set comm_num=comm_num+1 WHERE id='{$row['post_id']}'" );
